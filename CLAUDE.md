...
**Launch gating — Collect = "Pro" via manual request-access (shipped 2026-06-24):** Monetization stance for launch (user's call): **no automated billing yet.** Collect stays behind the existing single `CONSULTANT_PASSCODE` gate (`middleware.ts` + `/login`) so free users can't reach it; it's now *presented* as **Pro** with a **"Request Pro access"** CTA on the homepage Collect section (`LandingPage.tsx`) + a link on `/login`, both pointing at `REQUEST_ACCESS_URL` (new `src/lib/links.ts` — a `mailto:rahulu626@gmail.com` with a prefilled subject/body for now; **swap that one constant** for a Calendly / Tally / Google-Form URL when one exists). First customers are onboarded + billed **manually** (UPI/invoice); the shared passcode is acceptable for that handful. No public pricing grid (price discussed 1:1). **Deferred (next pieces):** (1) **automated billing** — Razorpay + per-user Supabase Auth + RLS (replaces the shared passcode) once a few are paying manually; (2) the **company-facing PDF redesign** — the user is designing it in Claude Design from a provided prompt (calm, plain-English, grouped tick-boxes, Evergreen & Ember), then implement in `src/lib/report-pdf.ts` (jsPDF).
