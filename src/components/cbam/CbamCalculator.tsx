    ...
    <div className="bg-white border border-line rounded-xl shadow-[0_1px_2px_rgba(16,33,26,0.05)] overflow-hidden">
      ...
      <div className="flex items-center gap-2.5 px-5 py-3.5 bg-tint border-b border-line">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-semibold text-ink leading-snug">Embedded-emissions screening</p>
          <p className="text-[11.5px] text-ink-muted leading-snug">Estimate the CBAM embedded emissions of a covered good from EU default values.</p>
        </div>
        <span className="text-[10px] font-semibold text-brand-700 bg-white border border-brand-700/20 px-2 py-0.5 rounded-full flex-shrink-0">
          Screening
        </span>
      </div>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid sm:grid-cols-3 gap-3.5">
          <label className="block sm:col-span-1">
            <span className="block text-[11px] font-semibold text-ink-muted uppercase tracking-[0.08em] mb-1.5">Covered good</span>
            <select
              value={goodId}
              onChange={(e) => setGoodId(e.target.value as CbamGoodId)}
              className="w-full h-10 px-3 text-[13.5px] text-ink bg-white border border-line rounded-lg focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-400 transition-colors"
            >
              {CBAM_GOODS.map((g) => (
          ...
          <label className="block">
            <span className="block text-[11px] font-semibold text-ink-muted uppercase tracking-[0.08em] mb-1.5">
              Production / export
            </span>
            <div className="flex items-center gap-1.5">
              <input
                ...
                placeholder="0"
                className="flex-1 min-w-0 h-10 px-3 text-[13.5px] font-mono tabular-nums text-ink bg-white border border-line rounded-lg focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-400 transition-colors placeholder:text-ink-muted/50"
              />
              <span className="text-[12px] text-ink-muted whitespace-nowrap min-w-[32px] text-right">{good.unit}</span>
            </div>
          </label>
          ...
          <label className="block">
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="flex-1 min-w-0 h-10 px-3 text-[13.5px] font-mono tabular-nums text-ink bg-white border border-line rounded-lg focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-400 transition-colors placeholder:text-ink-muted/50"
              />
              <span className="text-[12px] text-ink-muted whitespace-nowrap min-w-[64px] text-right">tCO₂e/{good.unit}</span>
            </div>
          </label>
        </div>
          ...
          {!hasInput ? (
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="rounded-xl bg-forest text-white px-4 py-3.5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-white/60">Total embedded emissions</p>
                <p className="mt-1.5 font-mono tabular-nums text-[26px] leading-none font-semibold">
                  {fmt(est.embeddedTco2e, 1)}
                  <span className="ml-1.5 text-[13px] font-sans font-normal text-white/70">tCO₂e</span>
                </p>
              ...
              <div className="rounded-xl bg-tint border border-line px-4 py-3.5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-ink-muted">Per-{good.unit} intensity</p>
                <p className="mt-1.5 font-mono tabular-nums text-[26px] leading-none font-semibold text-ink">
                  {fmt(est.perTonne, 3)}
                  <span className="ml-1.5 text-[13px] font-sans font-normal text-ink-muted">tCO₂e/{good.unit}</span>
                </p>
        ...
        <div className="text-[10.5px] text-ink-muted leading-relaxed space-y-1 border-t border-line pt-3">
          <p>
