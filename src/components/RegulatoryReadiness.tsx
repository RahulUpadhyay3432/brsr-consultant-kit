...
const VERDICT: Record<RegVerdict, { label: string; cls: string; dot: string }> = {
  applies:    { label: "Likely applies",   cls: "text-[#A8481B] bg-[#FBE7DC] border-[#F2CDBA]", dot: "bg-[#D9682E]" },
    ...
    <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-[0_1px_3px_rgba(80,60,30,0.04)]">
      ...
      <div className="px-5 py-4 border-b border-stone-100">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="font-display text-[19px] text-stone-900 leading-tight">{a.name}</h3>
            <p className="text-[12.5px] text-stone-500 mt-1.5 max-w-[64ch] leading-relaxed">{a.what}</p>
          </div>
          <span className={`flex-shrink-0 inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full border ${v.cls}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${v.dot}`} />{v.label}
          </span>
        </div>
        <div className={`mt-3 text-[12.5px] leading-relaxed rounded-lg px-3 py-2.5 border ${v.cls}`}>{a.reason}</div>
      </div>

      <div className="px-5 py-4 grid md:grid-cols-2 gap-5">
        {/* Timeline */}
        <div>
          <p className="text-[10.5px] font-semibold uppercase tracking-[0.1em] text-stone-500 mb-2.5">Key dates</p>
          <ul className="space-y-2.5">
            {a.timeline.map((t, i) => (
              <li key={i} className="flex gap-2.5">
                <span className="flex-shrink-0 mt-1 w-1.5 h-1.5 rounded-full bg-brand-500" />
                <div>
                  <p className="text-[12px] font-semibold text-stone-700 leading-snug">{t.date}</p>
                  <p className="text-[12px] text-stone-500 leading-snug">{t.event}</p>
                </div>
              </li>
            ))}
          </ul>
        ...
        {/* Checklist */}
        <div>
          <p className="text-[10.5px] font-semibold uppercase tracking-[0.1em] text-stone-500 mb-2.5">Readiness checklist</p>
          <ul className="space-y-2.5">
            {a.checklist.map((c, i) => (
              <li key={i} className="flex gap-2.5">
                ...
                <div>
                  <p className="text-[12.5px] font-semibold text-stone-700 leading-snug">{c.task}</p>
                  <p className="text-[12px] text-stone-500 leading-snug mt-0.5">{c.detail}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <div className="px-5 py-3 bg-stone-50/60 border-t border-stone-100">
        <p className="text-[11px] text-stone-400 leading-relaxed">{a.statusNote}</p>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
          <span className="text-[10.5px] font-semibold uppercase tracking-wide text-stone-400">Sources</span>
          {a.sources.map((s, i) => (
            <a key={i} href={s.url} target="_blank" rel="noreferrer" className="text-[11px] text-brand-700 hover:text-brand-800 underline decoration-stone-300 hover:decoration-brand-500 transition-colors">{s.label} ↗</a>
          ))}
        </div>
      </div>
    </div>
...
export default function RegulatoryReadiness({ regulatory }: { regulatory: RegulatoryReadiness }) {
  return (
    <div className="space-y-4">
      <div>
        <h1 className="font-display text-[24px] font-normal text-stone-900 leading-tight tracking-tight">Beyond BRSR</h1>
