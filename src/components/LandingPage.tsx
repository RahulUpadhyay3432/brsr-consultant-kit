...
interface HowStep { n: string; t: string; chips: string[]; act: string; out: string }
...
function StepCard({ s, i }: { s: HowStep; i: number }) {
  return (
    <div data-reveal style={{ transitionDelay: `${(i % 2) * 80}ms` }} className="card-lift rounded-2xl bg-white border border-line shadow-elev-1 p-5">
    ...
    <div className="min-h-screen bg-page text-ink">
      ...
      <section id="how" className="bg-white border-y border-line scroll-mt-24">
        <div data-reveal className="max-w-[1280px] mx-auto px-5 sm:px-8 py-20">
          ...
          {/* Mobile: single ordered stack (1 → 2 → 3 → 4) */}
          <div className="mt-10 space-y-5 md:hidden">
            {HOW_STEPS.map((s, i) => (
              <StepCard key={s.n} s={s} i={i} />
            ))}
          </div>
          {/* Desktop: two content-sized columns so card 3 sits directly under card 1 (no gap) */}
          <div className="mt-10 hidden md:grid md:grid-cols-2 gap-5 items-start">
            <div className="flex flex-col gap-5">
              {[HOW_STEPS[0], HOW_STEPS[2]].map((s, i) => (
                <StepCard key={s.n} s={s} i={i} />
              ))}
            </div>
            <div className="flex flex-col gap-5">
              {[HOW_STEPS[1], HOW_STEPS[3]].map((s, i) => (
                <StepCard key={s.n} s={s} i={i} />
              ))}
            </div>
          </div>
