# BRSR Consultant Kit — Project Context for Codex
...
> Maintenance note for Codex: keep this block + the rest of AGENTS.md current as features ship, and keep `[memory]/consultant-feedback-roadmap.md` in sync. This file is auto-loaded each session, so it is the primary way a new chat learns the current state.
